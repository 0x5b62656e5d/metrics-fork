...
export default async function({login, q, imports, data, account}, {enabled = false, token, extras = false} = {}) {
  ...
  try {
    ...
    const {data: {data: stats}} = isHackatime ? await imports.axios.get(`${url}/api/hackatime/v1/users/current/stats/${range}?api_key=${token}`) : await imports.axios.get(`${url}/api/v1/users/${user}/stats/${range}?api_key=${token}`)
